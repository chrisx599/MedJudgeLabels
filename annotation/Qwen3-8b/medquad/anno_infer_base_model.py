"""
标注脚本：使用基础Qwen3-8B模型对PKU数据进行标注（无LoRA适配器）
基于anno_prompt.txt的标注规则

功能:
- 加载基础Qwen3-8B模型（不使用LoRA适配器）
- 对pku_anno_formatted_test.jsonl进行推理
- 输出标注结果到base-model_pku_anno_formatted_test/目录
"""
import json
import torch
...
def main():
    ...
    parser.add_argument('--test_data_path', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/MedJudgeLabels/data/MedQuad_Dataset_aligned.jsonl',
                        help='测试数据路径')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/MedJudgeLabels/annotation/Qwen3-8b/medquad/base-model_medquad_test',
                        help='输出目录')
    ...
    # 3. 执行标注
    output_jsonl = os.path.join(args.output_dir, 'pku_anno_formatted_test_labels.jsonl')
    output_csv = os.path.join(args.output_dir, 'pku_anno_formatted_test_labels.csv')
