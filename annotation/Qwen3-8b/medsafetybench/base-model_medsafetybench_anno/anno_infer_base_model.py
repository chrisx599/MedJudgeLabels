"""
标注脚本：使用基础Qwen3-8B模型对Med Safety Bench数据进行标注（无LoRA适配器）
基于anno_prompt.txt的标注规则

功能:
- 加载基础Qwen3-8B模型（不使用LoRA适配器）
- 对med_safety_bench_formatted_test_cleaned.jsonl进行推理
- 输出标注结果到base-model_medsafetybench_anno/目录
"""
import json
import torch
...
def load_model(base_model_path, max_seq_length=2048, load_in_4bit=True):
    """
    加载基础模型（不使用LoRA适配器）
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='使用基础模型对数据进行标注（不使用LoRA适配器）')
    
    parser.add_argument('--base_model_path', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/scratchDirectory/Qwen3-8B',
                        help='基础模型路径')
    parser.add_argument('--test_data_path', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/MedJudgeLabels/data/med_safety_bench_formatted_test_cleaned.jsonl',
                        help='测试数据路径')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/MedJudgeLabels/annotation/Qwen3-8b/judgebench-medicalsafety/base-model_medsafetybench_anno',
                        help='输出目录')
    ...
    # 3. 执行标注
    output_jsonl = os.path.join(args.output_dir, 'med_safety_bench_formatted_test_labels.jsonl')
    output_csv = os.path.join(args.output_dir, 'med_safety_bench_formatted_test_labels.csv')
