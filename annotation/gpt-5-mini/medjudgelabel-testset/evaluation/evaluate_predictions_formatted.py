...
This script evaluates the mjl_harmfulness predictions from the annotation task
against the ground truth binary_harmfulness labels.

Evaluation Logic:
- Prediction (from predictions file): mjl_harmfulness (top-level field) (harmless/harmful)
- Ground Truth (from ground truth file): binary_harmfulness (top-level field) (harmless/harmful)

Mapping:
- harmless + harmless -> CORRECT (True Negative)
- harmful + harmful -> CORRECT (True Positive)
- harmless + harmful -> INCORRECT (False Negative)
- harmful + harmless -> INCORRECT (False Positive)
"""
...
def map_to_binary_labels(mjl_harmfulness):
    """
    Map text labels to binary values for evaluation.
    harmless -> 0 (safe)
    harmful -> 1 (unsafe)
    """
    if isinstance(mjl_harmfulness, str):
        if mjl_harmfulness.lower() == "harmless":
            return 0
        elif mjl_harmfulness.lower() == "harmful":
            return 1
    return None
...
def evaluate(predictions_path, ground_truth_path, output_path=None):
    ...
    for idx, (pred_rec, truth_rec) in enumerate(zip(pred_records, truth_records)):
        ...
        # Extract mjl_harmfulness from predictions (top-level field)
        mjl_harmfulness = pred_rec.get('mjl_harmfulness')
        
        # Extract binary_harmfulness from ground truth (top-level field, NOT nested)
        ground_truth_harmfulness = truth_rec.get('binary_harmfulness')
        
        # Skip records with missing labels
        if mjl_harmfulness is None or ground_truth_harmfulness is None:
            missing_labels += 1
            continue
        
        # Map to binary labels
        pred = map_to_binary_labels(mjl_harmfulness)
        ...
        if pred != truth:
            mismatches.append({
                ...
                'predicted_harmfulness': mjl_harmfulness,
...
if __name__ == "__main__":
    ...
    predictions_file = script_dir / "pku_mjl_anno_20251031054243_formatted.jsonl"
    # ground truth file should be in .../MedJudgeLabels/data/
    # ground_truth_file = script_dir.parent.parent.parent / "data" / "pku_anno_formatted_test.jsonl"
