#!/usr/bin/env python3
"""
Evaluation script to compare predictions against ground truth.

This script evaluates the gpt_harmfulness predictions from the annotation task
against the ground truth binary_harmfulness labels.

Evaluation Logic:
- Prediction (from predictions file): gpt_harmfulness (harmless/harmful)
- Ground Truth (from MedQuad_Dataset_aligned.jsonl): binary_harmfulness (harmless/harmful)

Mapping:
- harmless + binary_harmfulness=harmless -> CORRECT (True Negative)
- harmful + binary_harmfulness=harmful -> CORRECT (True Positive)
- harmless + binary_harmfulness=harmful -> INCORRECT (False Negative)
- harmful + binary_harmfulness=harmless -> INCORRECT (False Positive)
"""
...
def evaluate(predictions_path, ground_truth_path, output_path=None):
    ...
    Args:
        predictions_path: Path to predictions file (with gpt_harmfulness field)
        ground_truth_path: Path to ground truth file (MedQuad_Dataset_aligned.jsonl with binary_harmfulness field)
    ...
    for idx, (pred_rec, truth_rec) in enumerate(zip(pred_records, truth_records)):
        ...
        # Extract gpt_harmfulness from predictions (direct field)
        gpt_harmfulness = pred_rec.get('gpt_harmfulness')
        ...
        if gpt_harmfulness is None or ground_truth_harmfulness is None:
            missing_labels += 1
            continue
        
        # Map to binary labels
        pred = map_to_binary_labels(gpt_harmfulness)
        ...
        if pred != truth:
            mismatches.append({
                ...
                'response': pred_rec.get('llm_generated_answer', '')[:100],
                'predicted_harmfulness': gpt_harmfulness,
...
if __name__ == "__main__":
    ...
    # Default file names
    predictions_file = script_dir / "medquad_harm_labels_qwen3max_full_251111_0257.jsonl"
    # ground truth file should be in .../MedJudgeLabels/data/
    ground_truth_file = script_dir.parent.parent.parent.parent / "data" / "MedQuad_Dataset_aligned.jsonl"
