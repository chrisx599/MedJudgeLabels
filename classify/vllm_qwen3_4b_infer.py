#!/usr/bin/env python3
"""
vLLM Batch Inference Script for Medical Query Classification
使用Qwen3-4B模型进行医疗查询分类的批量推理脚本
"""
    ...
    def process_batch_outputs(batch_outputs, start_idx):
        ...
        for j, output in enumerate(batch_outputs):
            ...
            try:
                ...
                # 如果JSON解析失败，尝试从文本中提取label
                print(f"警告: 查询 {i} 的输出不是有效的JSON: {generated_text}")
                if "medical" in generated_text.lower():
                    label = "medical"
                elif "others" in generated_text.lower():
                    label = "others"
                else:
                    label = "unknown"
...
def main():
    parser = argparse.ArgumentParser(
        description="使用vLLM和Qwen3-4B进行医疗查询分类的批量推理"
    )
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="/common/home/projectgrps/CS707/CS707G2/Qwen3-4B-Instruct-2507",
        help="模型名称或路径 (默认: Qwen/Qwen2.5-3B-Instruct)"
    )
    parser.add_argument(
        "--prompt",
        type=str,
        default="classify/classify_prompt.txt",
        help="Prompt模板文件路径 (默认: classify/classify_prompt.txt)"
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    others_count = sum(1 for r in results if r['label'] == 'others')
    unknown_count = sum(1 for r in results if r['label'] == 'unknown')
    
    print("\n=== 分类统计 ===")
    print(f"总查询数: {len(results)}")
    print(f"医疗相关 (medical): {medical_count} ({medical_count/len(results)*100:.1f}%)")
    print(f"其他 (others): {others_count} ({others_count/len(results)*100:.1f}%)")
    if unknown_count > 0:
