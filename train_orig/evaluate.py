...
def classify_single(model, tokenizer, query, response, max_new_tokens=256, temperature=0.3):
    ...
    Returns:
        预测的is_response_safe (True/False)，如果解析失败返回None
    """
    # 格式化输入
    ...
    if result and 'is_response_safe' in result:
        return result['is_response_safe']
    else:
        return None
...
def evaluate_on_dataset(model, tokenizer, test_dataset, max_samples=None, save_predictions=None):
    ...
    for i, sample in enumerate(tqdm(test_dataset, desc="评估进度")):
        ...
        true_label = sample['is_response_safe']
...
def print_results(results):
    ...
    for label in results['labels']:
        ...
        label_str = str(label)
        print(f"{label_str:<15} {metrics['precision']:<12.4f} {metrics['recall']:<12.4f} "
    ...
    for label in labels:
        print(f"{str(label):<15}", end='')
    print()
    print("-" * 80)
    
    # 打印矩阵
    for i, true_label in enumerate(labels):
        print(f"{str(true_label):<15}", end='')
...
def main():
    ...
    parser.add_argument('--train_data_path', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/MedJudgeLabels/data/pku_anno_formatted_train_orig.jsonl',
                        help='训练集数据路径')
    parser.add_argument('--test_data_path', type=str,
                        default='/common/home/projectgrps/CS707/CS707G2/MedJudgeLabels/data/pku_anno_formatted_test_orig.jsonl',
                        help='测试集数据路径')
    ...
    # 如果没有指定LoRA适配器路径，使用默认值
    if args.lora_adapter_path is None:
        args.lora_adapter_path = './outputs/lora_adapter'
    ...
    print(f"训练集路径: {args.train_data_path}")
    ...
    # 2. 加载测试数据
    print("\n加载数据...")
    datasets = load_split_datasets(args.train_data_path, args.test_data_path)
